...
export default function WorldPage() {
  ...
  useEffect(() => {
    fetch('/api/novels')
      ...
      .then((data) => { setNovels(Array.isArray(data) ? data : []); if (Array.isArray(data) && data.length > 0) setSelectedNovel(data[0].id) })
      .catch((err) => console.error(err))
      .finally(() => setLoading(false))
  }, [])

  useEffect(() => { if (selectedNovel) fetchData() }, [selectedNovel, tab])

  const fetchData = async () => {
    try {
      const endpoint = { locations: 'locations', factions: 'factions', rules: 'world-rules', timeline: 'timeline' }[tab]
      const r = await fetch(`/api/novels/${selectedNovel}/${endpoint}`)
      if (!r.ok) return
  ...
  const handleSave = async () => {
    ...
    try {
      const endpoint = { locations: 'locations', factions: 'factions', rules: 'world-rules', timeline: 'timeline' }[tab]
      if (editingId) {
        await fetch(`/api/novels/${selectedNovel}/${endpoint}`, {
          method: 'PATCH',
  ...
  const handleDelete = async (id: string) => {
    const endpoint = { locations: 'locations', factions: 'factions', rules: 'world-rules', timeline: 'timeline' }[tab]
    await fetch(`/api/novels/${selectedNovel}/${endpoint}?id=${id}`, { method: 'DELETE' })
    fetchData()
  }
  ...
  return (
    <div className="mx-auto max-w-4xl px-8 py-10">
      ...
      {novels.length === 0 ? (
        <div className="text-center py-12"><Globe className="size-10 text-muted-foreground/30 mx-auto mb-3" /><p className="text-muted-foreground">请先创建一部小说</p></div>
      ) : (
        <>
          ...
          <div className="space-y-2">
            {items.map((item) => (
              <Card key={item.id} tab={tab} item={item} onEdit={() => handleEdit(item)} onDelete={() => handleDelete(item.id)} />
            ))}
...
function Card({ tab, item, onEdit, onDelete }: {
  tab: Tab; item: ItemBase; onEdit: () => void; onDelete: () => void
}) {
  const title = String(item[tab === 'rules' ? 'title' : 'name'] ?? '')
  const subtitle = String(item[tab === 'timeline' ? 'eventTime' : 'type'] ?? '')
  const body = String(item[tab === 'factions' ? 'goal' : 'description'] ?? item['content'] ?? '')

  return (
    <div className="rounded-lg border p-3 bg-card flex items-start gap-3 group">
      ...
      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        <button className="p-1 hover:bg-accent rounded" onClick={onEdit}><Pencil className="size-3.5 text-muted-foreground" /></button>
        <button className="p-1 hover:bg-accent rounded" onClick={onDelete}><Trash2 className="size-3.5 text-muted-foreground" /></button>
      </div>
    </div>
  )
}
